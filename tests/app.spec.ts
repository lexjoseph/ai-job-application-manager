...
test("should load the home page", async ({ page }) => {
  ...
  await expect(
    page.getByText("My Personal AI Job Application Tracker"),
  ).toBeVisible();
});
...
test("should add a new job application", async ({ page }) => {
  ...
  await expect(
    page.getByText("Test Company - Test Role - Applied - 2023-10-01"),
  ).toBeVisible();
...
test("should add multiple applications", async ({ page }) => {
  ...
  await expect(
    page.getByText("Company 2 - Role 2 - Applied - 2023-10-02"),
  ).toBeVisible();
  await expect(
    page.getByText("Company 1 - Role 1 - Applied - 2023-10-01"),
  ).toBeVisible();
