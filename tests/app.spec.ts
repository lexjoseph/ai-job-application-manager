...
test('should add a new job application', async ({ page }) => {
  ...
  await page.getByPlaceholder('date').fill('2023-10-01');

  // Submit the form
  await page.getByRole('button', { name: 'Add Application' }).click();

  // Check if the application appears in the list
  await expect(page.getByText('Test Company - Test Role - Applied - 2023-10-01')).toBeVisible();
...
test('should add multiple applications', async ({ page }) => {
  ...
  await page.getByPlaceholder('date').fill('2023-10-02');
  await page.getByRole('button', { name: 'Add Application' }).click();

  // Check both applications are in the list (in reverse order)
  await expect(page.getByText('Company 2 - Role 2 - Applied - 2023-10-02')).toBeVisible();
  await expect(page.getByText('Company 1 - Role 1 - Applied - 2023-10-01')).toBeVisible();
